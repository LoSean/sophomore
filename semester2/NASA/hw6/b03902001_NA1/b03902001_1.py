...
class DumbSwitch(app_manager.RyuApp):
	...
	def _packet_in_handler(self, ev):
		...
		print 'src:', src
		self.mactable[src] = inPort
		if dst in self.mactable:
			print 'outPort:', self.mactable[dst]
			self.addRule(datapath, dst, self.mactable[dst])
			if not dst == '00:00:00:00:00:03':
				self.sendPacket(datapath, msg, self.mactable[dst])
		else:
